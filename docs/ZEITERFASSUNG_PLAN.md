...
**Zeitkorrekturen.** Sascha kann Beginn und Ende ändern und Einträge nachträglich anlegen — beides **nur mit Begründung** (mindestens 5 Zeichen). Jede Änderung schreibt eine Zeile nach `time_edit_logs` (Feld, alter Wert, neuer Wert, Grund), der Eintrag bekommt `wasEdited = true` und ist in der Übersicht als korrigiert erkennbar.

**Löschen** verlangt dagegen **keine Begründung**, nur eine Bestätigung. Das ist bewusst anders als bei Korrekturen: Gelöscht wird fast immer aufgeräumt (Doppeleintrag, Fehlstempelung von Sekunden), und ein Pflichtfeld erzeugt dort erfahrungsgemäß nur Platzhaltertexte ohne Aussage. Der Vorgang selbst bleibt trotzdem protokolliert — mit Zeitpunkt und den Eckdaten des gelöschten Eintrags. Gelöschte Einträge erscheinen auf der Mitarbeiterseite in einem eigenen Abschnitt „Gelöschte Einträge"; ein Protokoll, das niemand sehen kann, nützt im Streitfall wenig.
