import { listListsAction } from "./scraping/actions";
import { FolderGrid } from "./_components/folder-grid";

export const dynamic = "force-dynamic";

export default async function VertriebPage() {
  const lists = await listListsAction();

  return (
    <div className="p-4 md:p-6 space-y-4 max-w-5xl">
      <div>
        <h2 className="text-base font-semibold text-ink">Listen</h2>
        <p className="text-xs text-sub mt-0.5">
          Deine Lead-Listen. Erstelle eine Liste und füll sie per Scrapen,
          CSV-Import oder manuell — danach pro Zeile mit Gemini anreichern.
        </p>
      </div>

      <FolderGrid lists={lists} />
    </div>
  );
}
